...
let _sndCtx;
function playNotif(){try{if(!_sndCtx)_sndCtx=new(window.AudioContext||window.webkitAudioContext)();_sndCtx.resume();const o=_sndCtx.createOscillator(),g=_sndCtx.createGain();o.frequency.value=880;o.type='sine';g.gain.value=0.1;o.connect(g);g.connect(_sndCtx.destination);o.start();o.stop(_sndCtx.currentTime+0.15);}catch(e){}}
...
.mc-toggle{position:absolute;top:8px;width:22px;height:22px;border-radius:50%;border:0.5px solid var(--mc-bd);background:#fff;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:12px;z-index:5}
.mc-side .mc-toggle{left:-11px}.mc-info .mc-toggle{right:-11px}
...
.mc-bact button{font-size:9px;padding:2px 6px;border-radius:6px;border:0.5px solid rgba(0,0,0,.1);cursor:pointer;background:rgba(255,255,255,.2)}
.mc-bbl.u .mc-bact button{color:#fff;border-color:rgba(255,255,255,.25)}
.mc-bbl.b .mc-bact button{color:var(--mc-t);background:#fff;border-color:var(--mc-bd)}
...
<span class="mc-toggle" id="mcSideToggle" title="Toggle sidebar">◀</span>
...
<span class="mc-toggle" id="mcInfoToggle" title="Toggle info">▶</span>
</div>
</div>
...
<div class="mc-ibtn ch-copy-conv" title="Copy conversation"><i class="ti ti-copy"></i><span>Copy</span></div>
<div class="mc-ibtn ch-reply-btn" title="Reply to message"><i class="ti ti-arrow-back-up"></i><span>Reply</span></div>
...
<div class="mc-hint">Press Ctrl+Enter to send</div></div>`);
	...
	initMic(){const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR){this.$el.find('.ch-mic').attr('title','Voice not supported in this browser').css('opacity','.3');return;}this.mic=new SR();this.mic.lang=frappe.boot.lang==='ar'?'ar-SA':'en-US';this.mic.interimResults=true;this.mic.continuous=true;this.mic.onresult=e=>{let t='';for(let i=0;i<e.results.length;i++)t+=e.results[i][0].transcript;this.$input.val(t).trigger('input');};this.mic.onend=()=>{this.$el.find('.ch-mic').removeClass('rec');this.$el.find('.ch-mic i').removeClass('ti-player-stop').addClass('ti-microphone');};this.mic.onerror=()=>{this.$el.find('.ch-mic').removeClass('rec');this.$el.find('.ch-mic i').removeClass('ti-player-stop').addClass('ti-microphone');};}
	toggleMic(){if(!this.mic)return;const $b=this.$el.find('.ch-mic');if($b.hasClass('rec')){this.mic.stop();}else{$b.addClass('rec');$b.find('i').removeClass('ti-microphone').addClass('ti-player-stop');try{this.mic.start();}catch(e){$b.removeClass('rec');$b.find('i').removeClass('ti-player-stop').addClass('ti-microphone');}}}
	...
	savePin(){if(!this.session)return;frappe.call('mubtkir_ai_creator.api.update_session_info',{session:this.session,title:this.pinned?'📌 '+this.pinned.substring(0,80):this.client});}
	loadPin(){/* pinned message is persisted via session title prefix */if(!this.session)return;frappe.call({method:'mubtkir_ai_creator.api.get_session_stats',args:{session:this.session},callback:r=>{const s=r.message||{};if((s.title||'').startsWith('📌 ')){this.pinned=s.title.substring(2).trim();this.$pin.find('.ptxt').text(this.pinned);this.$pin.show();}}});}
	...
	handleRes(res){if(!res)return;if(res.type==='message')return this.addB('b',frappe.utils.escape_html(res.text||''),res.text);if(res.type==='approval_required'){this.addB('b',frappe.utils.escape_html(res.plan||''),res.plan);const rl=res.risk_level;const $box=$(`<div style="border:0.5px solid var(--mc-bd);border-radius:8px;padding:10px;margin-bottom:6px"><div style="margin-bottom:6px;font-weight:500;font-size:11px;color:#92400e">Risk: ${rl} — Approval required</div><pre style="max-height:150px;overflow:auto;font-size:9px;direction:ltr;text-align:left;background:var(--mc-sf);padding:6px;border-radius:6px">${frappe.utils.escape_html(JSON.stringify(res.calls,null,2))}</pre><div style="display:flex;gap:6px;margin-top:6px"><button class="appr mc-send" style="font-size:10px;padding:4px 12px">Approve</button><button class="rej mc-hdr-btn" style="font-size:10px">Reject</button></div></div>`);$box.find('.appr').on('click',()=>this.approve(res.task,$box));$box.find('.rej').on('click',()=>this.reject(res.task,$box));this.$msgs.append($box);this.$msgs.scrollTop(this.$msgs[0].scrollHeight);}}
