...
def run_import(import_name, start_row=0):
    ...
    # Detect if the remote doctype has autoname (name generated by rule)
    doctype_autoname = ""
    try:
        meta_resp = client.get_meta(doc.remote_doctype)
        doctype_autoname = (meta_resp.get("data") or {}).get("autoname", "") or ""
    except Exception:
        pass
    ...
    for batch_num in range(total_batches):
        ...
        for row_idx, row in enumerate(batch_rows):
            ...
            try:
                row_data = _build_row_data(headers, row, mapping, doc, doctype_autoname)
...
def _build_row_data(headers, row, mapping, import_doc, doctype_autoname=""):
    ...
    # If doctype has autoname, skip name field to let the server generate it
    skip_name = bool(doctype_autoname)
    ...
    for col_idx, header in enumerate(headers):
        ...
        # Skip name field when doctype uses auto-naming
        if skip_name and target == "name":
            continue
